...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.port_combo = QComboBox(); self.port_combo.addItem("🔧 Simulated Data", None)
        for port,desc in list_serial_ports(): self.port_combo.addItem(f"{port} ({desc})", port)
        ...
        for i in list_cameras(4): self.cam_combo.addItem(f"Camera {i}",i)
        if self.cam_combo.count()==0: self.cam_combo.addItem("No cameras found",None)
        tb.addWidget(self.cam_combo)

        # Status bar
        self.status=QStatusBar();self.setStatusBar(self.status)
        self._elapsed=0; self._timer=QTimer(self)
        self._timer.timeout.connect(self._update_status); self._timer.start(1000)
    ...
    def resizeEvent(self,event):
        ...
        if self._last_pixmap: self._update_video_label()
    ...
    def _on_frame(self,img,frame_bgr):
