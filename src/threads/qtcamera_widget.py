import logging
import imagingcontrol4 as ic4  # For the DeviceInfo type
...
class QtCameraWidget(QWidget):
    """
    Displays live camera feed via SDKCameraThread into a GLViewfinder.
    Manages camera selection, resolution, and debounced exposure/gain.
    """
    ...
    def set_active_camera_device(self, device_info=None):
        log.info(f"Selecting camera: {getattr(device_info, 'model_name', None)}")
    ...
    @pyqtSlot(str)
    def set_active_resolution_str(self, res_str: str):
        if "x" not in res_str:
            return
        try:
            w_s, h_rest = res_str.split("x", 1)
            h_s = h_rest.split()[0]
            w, h = int(w_s), int(h_s)
            ...
            log.error(f"Bad resolution string: {res_str}")
    ...
    def _on_sdk_frame_received(self, qimg, frame_data):
        if qimg and not qimg.isNull():
            # push into your GL widget
            self.viewfinder.update_frame(qimg)
            self.frame_ready.emit(qimg, frame_data)
        else:
            log.warning("Null QImage from camera thread")

    @pyqtSlot(str, str)
    def _on_camera_thread_error_received(self, msg, code):
        log.error(f"Camera error {code}: {msg}")
        self.camera_error.emit(msg, code)
