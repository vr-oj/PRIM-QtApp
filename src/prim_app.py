...
try:
    ...
    log = logging.getLogger(__name__)
    log.warning(
        f"Could not import from config.py: {e}. Using default INFO level for logging."
    )
...
try:
    ...
    IC4_AVAILABLE = False
    log.warning(
        "imagingcontrol4 library not found. TIS camera functionality will be unavailable."
    )
...
def main_app_entry():
    ...
    if IC4_AVAILABLE:
        ...
        except Exception as e_ic4_init:
            log.error(f"Failed to initialize imagingcontrol4 Library: {e_ic4_init}")
            # Optionally, inform the user via QMessageBox if TIS camera is critical
            # QMessageBox.critical(None, "Camera Library Error", f"Failed to initialize TIS camera library: {e_ic4_init}")
            # IC4_AVAILABLE = False # Treat as unavailable if init fails
    ...
    try:
        ...
        def handle_exception(exc_type, exc_value, exc_traceback):
            ...
            error_dialog.setInformativeText(
                "Please check the console output or log file for more details."
            )
            error_dialog.exec_()
        ...
        if os.path.exists(qss_file_path):
            try:
                ...
                log.warning(
                    f"Could not load or apply style.qss from {qss_file_path}: {e_qss}"
                )
                app.setStyle(QStyleFactory.create("Fusion"))
        else:
            log.warning(
                f"Stylesheet style.qss not found at {qss_file_path}. Using default Fusion style."
            )
            app.setStyle(QStyleFactory.create("Fusion"))
        ...
        if IC4_AVAILABLE:
            ...
            def cleanup_ic4_library():
                log.info("Attempting to exit imagingcontrol4 Library...")
                try:
                    # Ensure all camera threads are properly stopped and joined before this.
                    # MainWindow.closeEvent should handle stopping its camera threads.
                    ic4.Library.exit()
                    log.info("imagingcontrol4 Library exited successfully.")
                except Exception as e_ic4_exit:
                    log.error(
                        f"Error during imagingcontrol4 Library.exit(): {e_ic4_exit}"
                    )

            app.aboutToQuit.connect(cleanup_ic4_library)
        ...
        try:
            QMessageBox.critical(
                None,
                f"{APP_NAME} - Fatal Startup Error",
            ...
            print(
                f"CRITICAL STARTUP ERROR (QMessageBox failed: {e_msgbox}):\n{detailed_error}"
            )
        sys.exit(1)
