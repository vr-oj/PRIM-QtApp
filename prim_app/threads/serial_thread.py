...
from utils.config import DEFAULT_SERIAL_BAUD_RATE, SERIAL_COMMAND_TERMINATOR
...
class SerialThread(QThread):
    ...
    def __init__(
        self, port=None, baud=DEFAULT_SERIAL_BAUD_RATE, test_csv=None, parent=None
    ):  # Added parent
        ...
        self.running = False
        self.fake_t_idx = (
            0  # Renamed from fake_t for clarity (it's an index/frame count)
        )
        self.test_csv = (
            test_csv  # test_csv functionality not fully implemented here, but retained
        )
        self.start_time = 0
            ...
            if self.ser:
                try:
                    if self.ser.in_waiting > 0:
                        ...
                        ).strip()  # Use replace for robustness
                        log.debug(f"Raw serial data: {line}")

                        parts = [fld.strip() for fld in line.split(",")]
                        if (
                            len(parts) < 3
                        ):  # Expecting at least 3 parts: time, frame_idx, pressure
                            log.warning(
                                f"Malformed data line: {line}. Expected 3 parts, got {len(parts)}"
                            )
                            continue
                        ...
                        try:
                            ...
                            t_device = float(
                                parts[0]
                            )  # This could be device's own timestamp or uptime
