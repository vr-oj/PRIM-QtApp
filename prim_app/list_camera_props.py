...
# ─── Add this minimal QueueSinkListener subclass ────────────────────────────────
class _MinimalSinkListener(ic4.QueueSinkListener):
    """
    A no-op implementation of QueueSinkListener.
    Just satisfies the abstract methods so we can construct a QueueSink.
    """
    ...
    def sink_connected(self, sink, connected: bool):
        # Likewise, no action needed on connect/disconnect events.
        pass
...
class IC4CameraController:
    ...
    def open(self):
        """
        1. Initialize IC4.
        2. Enumerate devices and pick the first matching DMK 33UX250 or 33UP5000.
        ...
        # Pick first preferred model, else fallback
        chosen_info = None
        for info in devices:
...
class CameraAppMainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Spacer so controls stay at the top
    ...
    def closeEvent(self, event):
        """Ensure we disconnect the camera when the window closes."""
        self.on_disconnect()
        event.accept()
