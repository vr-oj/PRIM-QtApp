...
char receivedChar;
bool newData;
bool isMaster = false;    // master mode when controlled via serial commands
...
void bootup () {
  ...
  printWords(0, 1, 120, 117, ST77XX_RED, "v3.04_master_clock");
...
// handle MASTER_ON/MASTER_OFF/TICK commands
void checkMasterCommand() {
  if (Serial.available()) {
...
void sampleAndSend() {
  unsigned long arduino_ts = micros();
  hiLow();
  averagingPressure();
  Serial.print(arduino_ts);
  Serial.print(',');
  Serial.println(avg);
}
