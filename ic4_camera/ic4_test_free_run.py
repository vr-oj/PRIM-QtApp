...
def main():
    ...
    # 3) Disable hardware trigger (if present)
    try:
        trigger_node = grabber.device_property_map.find_enumeration(
            ic4.PropId.TRIGGER_MODE
        )
        if trigger_node:
            available = [entry.name for entry in trigger_node.entries]
            print("  → Available TriggerMode entries:", available)
            if "Off" in available:
                trigger_node.value = "Off"
                print("  → TriggerMode set to Off")
            else:
                print("  → No 'Off' in TriggerMode entries; skipping trigger disable")
        else:
            print("  → No TriggerMode node found; assuming free-run is default")
    except Exception as e:
        print("  ✗ Could not set TriggerMode:", e)
    ...
    try:
        ...
        if w_node and h_node:
            ...
            print(f"  → Resolution set to {w_node.value}×{h_node.value}")
    except Exception as e:
        print("  ✗ Could not set resolution:", e)

    # 5) Create sink and listener
    listener = DummyListener()
    sink = ic4.QueueSink(listener, [ic4.PixelFormat.Mono8], max_output_buffers=1)
    print("  → QueueSink created.")

    # 6) Setup stream (requesting acquisition start here)
    grabber.stream_setup(sink, setup_option=ic4.StreamSetupOption.ACQUISITION_START)
    print("  → stream_setup() succeeded; acquisition should now be active.")

    # 7) Warm up for 2 seconds
    print("  → Sleeping 2 seconds for camera warm-up…")
    time.sleep(2.0)

    # 8) Try to pop 5 frames
    ...
    for i in range(5):
        try:
            ...
            arr = buf.numpy_wrap()
            print(f"    → Received frame {i+1}, shape={arr.shape}")
            cv2.imshow("Frame", arr)
            cv2.waitKey(200)
            sink.queue_buffer(buf)
        except ic4.IC4Exception as e:
            print(f"    ✗ pop_output_buffer() error on frame {i+1}:", e)
            time.sleep(0.1)
